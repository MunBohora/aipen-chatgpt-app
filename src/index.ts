...
function createServer(assetOrigin: string, clientId: string) {
  ...
  registerAppTool(
    ...
    "make_handwriting",
    {
      ...
      description: "Renders text as handwritten notes with compact JPEG previews and a vector PDF using Aipen. When the user asks for notes or selects an Aipen template, write the actual requested content and call this tool; do not respond with generic text instead. The anonymous demo is limited to five pages per request.",
      inputSchema,
      outputSchema,
      _meta: {
